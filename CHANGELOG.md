...
- 移除概览区域的圆角卡片描边，让实时指标直接通过图标、数字和留白形成层次
- 移除 Library 的深色外框、表格横向分隔线和操作栏分隔线，改为单一柔和面层与自绘轻量选中背景
- 主操作栏精简为 Run、Stop 和 Actions；Open Log、移除保存记录和磁盘删除进入分组菜单
- 主按钮随状态变化：停止或挂起 VM 时突出 Run，运行中 VM 时突出 Stop；多选仍保持批量能力提示
- Actions 在 Tart 不可用时仍可打开本地日志和移除失效记录，同时精确禁用依赖 Tart 的菜单命令

### Fixed

- 不再让长期禁用的 Remove Record 占据主界面最远端位置，也不再把日志、危险操作和生命周期操作表现为同一优先级
- 状态感知按钮在表格自动恢复排序和选择期间可安全刷新，不依赖尚未创建的控件

### Verified

- 使用运行、停止和挂起三种隔离 VM 状态验收无描边布局、Actions 菜单以及 Run/Stop 动态主按钮

## 5.1.0 - 2026-07-21

### Changed

- 重新设计主窗口的信息架构：品牌与主操作、VM 总数/运行数/主机可用空间概览，以及统一的虚拟机 Library 工作区形成清晰的三级层次
- 手动记录输入、VM 表格和上下文操作栏合并到同一卡片；低频操作不再与主要内容争夺视觉空间
- VM 名称使用带底色的设备图标，运行状态升级为低饱和彩色徽章，按钮密度、行高、间距、边框透明度和窗口安全区全部重新校准
- 使用自绘低饱和选中背景取代系统高亮蓝色，激活与非激活窗口中均保持名称、状态徽章和开关的可读性
- 正常状态不再重复显示与概览卡片相同的摘要；仅在筛选、多选、同步错误或宿主空间不足时显示上下文信息

### Verified

- 使用隔离 Tart 数据完成英文、简体中文、激活与非激活窗口的真实 AppKit 截图验收
- 保持多选、排序、搜索、状态同步、自动启动、全部 VM 操作和辅助功能语义不变

## 5.0.0 - 2026-07-20

### Added

- 完整支持 Tart 2.32.1 的 `tart set` 参数：CPU、内存、显示分辨率、显示自动适配开关、随机 MAC、随机 macOS 序列号、替换磁盘和磁盘扩容
- 配置面板新增三态 Display Refit、系统磁盘文件选择器和身份随机化选项；包含空格的 VM 名称与磁盘路径仍作为独立进程参数安全传递
- CLI 兼容门禁检查所有新增 `tart set` 选项，单元测试覆盖完整参数、启用显示适配和无变更命令

### Changed

- 主窗口升级为现代卡片式原生 macOS 界面：透明统一标题栏、视觉材质背景、强调色主操作、SF Symbols、状态色、搜索框和自动启动开关
- VM 列表使用更宽松的行距、图标化名称、彩色状态点和更清晰的操作层级，同时保持键盘、多选、排序与辅助功能支持
- 默认窗口扩大至 980×650，并为小屏幕保留 820×560 的可用最小尺寸

### Security

- 替换 VM 磁盘必须从系统文件选择器选择仍然存在的普通文件，并在执行不可撤销的覆盖前进行 critical 二次确认
- 所有 `tart set` 值继续通过 `Process.arguments` 传递，不使用宿主机 Shell 拼接

## 4.18.0 - 2026-07-20

### Added

- 完整支持 Tart 的 `TART_HOME`：可从 TartR 菜单选择并记住任意本地或外置卷目录
- 未设置 App 目录时自动继承启动环境中的 `TART_HOME`，否则保持 Tart 默认行为；运行环境和隐私诊断会显示当前生效来源
- 不同 Tart 主目录分别保存 VM 列表设置、自动启动、SSH 用户名和运行选项，切换目录不会混合配置
- 单元测试覆盖 App 设置、环境变量和默认值的优先级，以及向所有 Tart 子进程传递环境变量
- 双语打包冒烟测试使用持久化 App 设置，验证 `tart list` 与 `tart run` 收到完全一致的 `TART_HOME`

### Fixed

- 镜像克隆、归档导入、VM 创建和状态摘要的可用空间检查改为实际 `TART_HOME` 所在卷，而不是固定检查用户主目录
- 有 TartR 托管的 VM、修改任务、更新下载或状态同步进行时禁止切换主目录，避免跨目录误操作
...
- 完整支持英文和简体中文，应用自动跟随 macOS 首选语言
- 主窗口、菜单、状态、弹窗、错误、更新、磁盘预检、设置和辅助功能文案均纳入统一 `.strings` 资源
- 新增 343 个本地化 key 的自动覆盖检查，并校验 `%@`、`%d` 格式占位符一致性
- 打包冒烟测试分别以英文和简体中文启动应用，发布校验确认两套资源均封装进 ZIP 和 DMG
