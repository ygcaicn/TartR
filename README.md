...
TartR 是一个现代化的原生 macOS Tart 虚拟机管理器。它提供实时状态同步、官方镜像下载和常见虚拟机操作，同时保留 Tart CLI 的可预测行为。

## 功能

- 自动发现本地 Tart VM，并同步运行、停止和挂起状态
- 精细化原生 Dashboard：VM 总数、运行数量和主机可用空间概览，统一的 Library 列表与上下文操作栏
- 完整支持英文和简体中文，自动跟随 macOS 首选语言
- 支持 `TART_HOME` 环境变量，也可从 App 菜单选择并记住自定义 Tart 数据目录
...
- 完整支持 `tart set`：调整 CPU、内存、显示分辨率、显示自动适配和磁盘大小，替换磁盘内容，以及随机化 MAC 地址和 macOS 序列号
...
选中已停止的 VM 后，可从“更多操作…”→“调整配置…”管理 `tart set` 的全部硬件、显示、磁盘与身份参数。替换磁盘必须通过系统文件选择器选择现有文件，并在执行前再次确认；磁盘扩容仍遵循 Tart 只能增大的限制。
...
如果 VM 数据位于自定义目录（例如外置卷），可在 TartR 菜单选择“选择 Tart 主目录…”，然后选择 `/Volumes/mini/home/tart_data`。TartR 会记住该目录，并为所有 `tart list`、`tart run`、克隆、导入和其他 Tart 命令设置：

```bash
TART_HOME=/Volumes/mini/home/tart_data
```

优先级为 App 中保存的目录、启动环境中的 `TART_HOME`、Tart 默认目录。不同主目录分别保存 VM 自动启动、运行选项和 SSH 用户名；切换前必须停止由 TartR 启动的 VM 并等待当前任务结束。运行环境页面会显示当前路径和来源。

TartR 会在状态摘要中显示当前 `TART_HOME` 所在卷的可用空间。下载镜像、从 IPSW 创建 VM、导入归档和导出归档前会预留额外 5 GB；空间不足时默认取消，也可以选择清理 Tart 缓存或在确认风险后继续。
...
```bash
make test
make localization # 校验全部中英文 key 与格式占位符
...
TartR 使用发布日期作为版本号：App 版本为 `YY.MM.DD`，GitHub 标签为 `vYY.MM.DD`，构建号为 `YYYY.M.D`。例如 2026 年 7 月 21 日发布 `v26.07.21`，对应构建号 `2026.7.21`。每个日期只发布一个稳定版本，`make version` 和 Release workflow 会拒绝旧格式、无效日期、错误构建号或不匹配的标签。
...
推送与版本一致的标签会由 Release workflow 自动发布。如果仓库已配置 Apple Secrets，工作流会完成 Developer ID 签名与公证；如果凭据尚未配置，则发布经过完整校验的 ad-hoc 签名构建并在 Release 中显示明确警告。详见 `docs/RELEASING.md`。
...
- App 选择的 `TART_HOME` 必须是可访问目录，并作为独立进程环境变量传递，不拼接进 shell 命令
